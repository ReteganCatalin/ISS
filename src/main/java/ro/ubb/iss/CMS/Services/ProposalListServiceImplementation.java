...
public class ProposalListServiceImplementation implements ProposalListService {
  private static final Logger log =
      LoggerFactory.getLogger(ProposalListServiceImplementation.class);

  @Autowired private ProposalListRepository proposalListRepository;
  ...
  @Override
  public Optional<ProposalList> saveProposal(ProposalListKey proposalKey) {
    log.trace("saveProposal - method entered: proposalListKey={}", proposalKey);
    ProposalList newProposalList = ProposalList.builder().proposalListKey(proposalKey).build();
    Optional<ProposalList> checkForPresence = proposalListRepository.findById(proposalKey);

    checkForPresence.ifPresentOrElse(
        analysis -> {}, () -> proposalListRepository.save(newProposalList));
