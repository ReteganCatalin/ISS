...
public class ParticipationServiceImplementation implements ParticipationService {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(ParticipationServiceImplementation.class);
  ...
  public Participation updateParticipation(
      int participantListID, Section section, User user, Role role) {
    log.trace(
        "updateParticipation - method entered: participantListID={}, section={}, user={}, role={}",
        participantListID,
        section,
        user,
        role);

    Optional<Participation> participationOptional =
        participationRepository.findById(participantListID);
  ...
  public Participation saveParticipation(Section section, User user, Role role) {
    log.trace(
        "saveParticipation - method entered: section={}, user={}, role={}", section, user, role);
    Participation newParticipation =
        Participation.builder().section(section).user(user).role(role).build();
