...
public class RoleForUserServiceImplementation implements RoleForUserService {
  ...
  private static final Logger log = LoggerFactory.getLogger(RoleForUserServiceImplementation.class);

  @Autowired RoleForUserRepository roleForUserRepository;

  @Override
  public Optional<RoleForUser> findRoleForUser(RoleForUserKey roleForUserKey) {
    log.trace("findRoleForUser - method entered roleForUserKey={}", roleForUserKey);
  ...
  public Optional<RoleForUser> saveRoleForUser(RoleForUserKey roleForUserKey) {
    log.trace("saveRoleForUser - method entered: permissionForUserKey={}", roleForUserKey);
    RoleForUser newRoleForUser = RoleForUser.builder().roleForUserKey(roleForUserKey).build();
    Optional<RoleForUser> checkForPresence = roleForUserRepository.findById(roleForUserKey);
