...
public class AbstractConverter implements BaseConverter<Abstract,AbstractDto> {
    ...
    public Abstract convertDtoToModel(AbstractDto abstractDto) {
        return Abstract.builder().abstractID(abstractDto.getAbstractID())
                .byteFileLocation(abstractDto.getByteFileLocation()).format(abstractDto.getFormat()).build();
    }

    @Override
    public AbstractDto convertModelToDto(Abstract anAbstract) {
        return AbstractDto.builder().abstractID(anAbstract.getAbstractID()).byteFileLocation(anAbstract.getByteFileLocation()).format(anAbstract.getFormat()).build();
    }
}
